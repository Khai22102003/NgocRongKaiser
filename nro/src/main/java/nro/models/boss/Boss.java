...
public abstract class Boss extends Player implements BossInterface {
    ...
    public int injured(Player plAtt, int damage, boolean piercing, boolean isMobAttack) {
        ...
        if (this.isDie()) {
            ...
            // Kiểm tra kỹ năng và giảm sát thương nếu kỹ năng thuộc ba kỹ năng cụ thể
            if (plAtt != null /* && Util.isTrue(1, 5) */) {  // Tạm thời bỏ ngẫu nhiên
                switch (plAtt.playerSkill.skillSelect.template.id) {
                    case Skill.TU_SAT:
                    case Skill.QUA_CAU_KENH_KHI:
                    case Skill.MAKANKOSAPPO:
                        damage = (damage * 80) / 100; // Giảm sát thương 20%
                        break;
                    default:
                        // Không giảm sát thương cho các kỹ năng khác
                        break;
                }
            }

            System.out.println("Sát thương trước khi điều chỉnh: " + damage);
            System.out.println("Boss còn sống: " + !this.isDie());
            System.out.println("ID Kỹ năng: " + (plAtt != null ? plAtt.playerSkill.skillSelect.template.id : "null"));

            // Áp dụng sát thương đã được điều chỉnh
            dame = super.injured(plAtt, damage, piercing, isMobAttack);

            System.out.println("Sát thương sau khi điều chỉnh: " + dame);
